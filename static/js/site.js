...
(function () {
  ...
  /* ================================================================
     ...
     Kabína cestuje k tomu poschodiu, ktorého sekcia je práve
     na obrazovke. Displej ukazuje číslo sekcie — rovnaké, aké je
     pri nadpise v obsahu, takže je jasné, kde sa čitateľ nachádza.
     ================================================================ */

  var floorsNav = document.querySelector('[data-floors]');

  if (floorsNav && 'IntersectionObserver' in window) {
    var car = floorsNav.querySelector('[data-floor-car]');
    var display = floorsNav.querySelector('[data-floor-display]');
    var links = Array.prototype.slice.call(floorsNav.querySelectorAll('.floors__link'));

    var stops = links
      .map(function (link) {
        var id = link.getAttribute('href').slice(1);
        return { link: link, num: link.dataset.floor, section: document.getElementById(id) };
      })
      .filter(function (s) {
        return s.section;
      });
    ...
    if (stops.length) {
      // Zámerne undefined, nie null: prvé volanie moveTo(null) musí prejsť,
      // aby kabína dostala počiatočnú pozíciu na prízemí.
      var current;
      ...
      function moveTo(stop) {
        ...
        if (!stop) {
          // Nad prvou sekciou — kabína stojí na prízemí.
          car.style.top = '0px';
        ...
        display.textContent = stop.num;
        car.style.top = stop.link.offsetTop + stop.link.offsetHeight / 2 + 'px';
      }

      var visible = new Set();

      var floorObserver = new IntersectionObserver(
        function (entries) {
          entries.forEach(function (entry) {
          ...
          // Aktívne je prvé poschodie, ktoré je práve v zábere.
          var active = null;
          for (var i = 0; i < stops.length; i++) {
            if (visible.has(stops[i].section)) {
              active = stops[i];
              break;
            }
          }
      ...
      stops.forEach(function (s) {
        floorObserver.observe(s.section);
      });
